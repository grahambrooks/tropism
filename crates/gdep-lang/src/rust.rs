//! Rust provider.
//!
//! Implemented so gdep can be run against itself. Rust is the third ecosystem and
    ...
    fn resolve_import(
        ...
        match root {
            ...
            _ => {
                ...
                // this reported gdep-core's own modules as undeclared dependencies.
...
/// cycle in gdep-core — and would in essentially every Rust crate ever written.
...
fn parse_cargo_toml(path: &Utf8Path, text: &str) -> anyhow::Result<Manifest> {
    ...
    // configuration. Extracting them regardless of the cfg is deliberate: gdep does
...
fn collect(node: tree_sitter::Node<'_>, source: &[u8], out: &mut Vec<Import>) {
    ...
    // `eprintln!("{}", gdep_core::report::S)` are both real uses that the structured
...
"#;
    ...
    /// *local* module with no `crate::` prefix. Missing this reported gdep-core's own
    ...
    /// modelling that as a dependency reported a cycle in gdep-core itself.
    #[test]
    ...
    fn paths_inside_macros_and_attributes_count_as_usage() {
        let imports = extract_rust_imports(
            ...
            "#[derive(Debug, thiserror::Error)]\nstruct E;\n\
             fn f() { eprintln!(\"{}\", gdep_core::report::S); }\n",
        )
        .unwrap();
        let roots: Vec<&str> = imports.iter().map(|i| i.raw.as_str()).collect();
        assert!(
