...
pub const RULESET_FILE: &str = "gdep.toml";
...
impl Ruleset {
    /// Loads `gdep.toml` from `scan_root`, if present.
...
"#;
    ...
    fn ruleset() -> Ruleset {
        Ruleset::parse(Utf8PathBuf::from("gdep.toml"), RULES).unwrap()
    }
    ...
    fn parse_error(text: &str) -> String {
        match Ruleset::parse(Utf8PathBuf::from("gdep.toml"), text) {
