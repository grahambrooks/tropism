...
//! narrow, filtered, and paginated, and `tropism_summary` is the cheap entry point.

fn main() -> std::process::ExitCode {
    eprintln!(
        "tropism-mcp is not implemented yet (build-order step 6).\n\
         The tool surface is specified in design/05-interfaces.md.\n\
         Schema version the server will speak: {}",
        tropism_core::report::SCHEMA_VERSION
    );
